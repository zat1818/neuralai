          ...
          <section>
            ...
            <div className="space-y-4">
              {Array.isArray(notifications) && notifications.map((notif: any) => (
                <NotificationItem key={notif.id} type={notif.type} text={notif.text} time={notif.createdat || notif.createdAt} />
              ))}
            ...
            <tbody className="divide-y divide-neural-red/10 text-xs font-mono">
              {signals.map((sig: any) => (
                <SignalRow 
                  ...
                  time={new Date(sig.createdat || sig.createdAt).toLocaleTimeString()} 
                  pnl={sig.pnl || '--'} 
                />
              ))}
              {signals.length === 0 && (
